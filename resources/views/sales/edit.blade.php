...
<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6 bg-white border-b border-gray-200">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Editar Venta</h2>
        ...
        <form action="{{ route('sales.update', $sale) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="pedido_id" id="pedido_id" 
                        ...
                        @foreach($pedidos as $pedido)
                            <option value="{{ $pedido->id }}" {{ old('pedido_id', $sale->pedido_id) == $pedido->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="fecha" id="fecha" value="{{ old('fecha', $sale->fecha) }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    @error('fecha')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="number" step="0.01" name="total" id="total" value="{{ old('total', $sale->total) }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    @error('total')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
