        ...
        <form action="{{ route('employees.update', $employee) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="ci" id="ci" value="{{ old('ci', $employee->ci) }}" 
                ...
                <div>
                    ...
                    <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $employee->nombre) }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    @error('nombre')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="rol" id="rol" 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        <option value="admin" {{ old('rol', $employee->rol) == 'admin' ? 'selected' : '' }}>Administrador</option>
                        <option value="cocinero" {{ old('rol', $employee->rol) == 'cocinero' ? 'selected' : '' }}>Cocinero</option>
                        <option value="mesero" {{ old('rol', $employee->rol) == 'mesero' ? 'selected' : '' }}>Mesero</option>
                    </select>
                    @error('rol')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
